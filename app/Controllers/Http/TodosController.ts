...
export default class TodosController {
    ...
    public async byUserId({view, auth}: HttpContextContract)
    {
    ...
    public async update({view, request, params}: HttpContextContract)
    {
        const todo = await Todo.find(params.id);
    ...
    {
       const user = await auth.authenticate();
        await Todo.query().where('user_id', user.id).where('id', params.id).delete();
